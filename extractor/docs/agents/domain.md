...
| Source document intake | The step that turns the command line's path-or-stdin spec into a source document, or into one named input failure — unreadable path, undecodable bytes, or a document past the character ceiling. Its depth is the guarantee: a returned document has already cleared the ceiling, so the limit is structural rather than an ordering rule `main` has to remember. Not a seam — there is one implementation and no reason to substitute it. |